        ...
        public static bool SendEmail(string txtBody, string txtEmailSender, string txtEmailSubject, string txtEmailTo, string txtJoinString, KampusMerdekaEntities dObjContext, DbContextTransaction dObjTran)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(txtEmailTo))
                {
                    ...
                    SmtpClient client = new SmtpClient(mSystemConfigurationCustomBL.GetmSystemConfigurationValue(Configuration.MODULE_NAME, Configuration.Key.SMTP, Configuration.DefaultValue.DefaultLangID, dObjContext, dObjTran));
                    ...
                    if (mSystemConfigurationCustomBL.GetmSystemConfigurationBoolean(Configuration.MODULE_NAME, Configuration.Key.bDebugEmail, Configuration.DefaultValue.DefaultLangID, dObjContext, dObjTran))
                    {
                        clsTrDebugEmailBL.SaveTrDebugEmail(DebugDat, string.Empty, Configuration.DefaultValue.DefaultLangID, Guid.NewGuid().ToString(), dObjContext, dObjTran);
                    }
                }
            }
            catch (Exception exx)
            {
                clsMMainCustomBL.SendEmailError(exx, "", Configuration.DefaultValue.DefaultLangID, "", mUserCustomBL.CreateBlankmUser(), dObjContext, dObjTran);
                //Throw exx
                return false;
            }
