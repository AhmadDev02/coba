...
namespace KN_KAMPUS_MERDEKA.BUSSLOGIC.CustomBL.Systems
{
    public static class mMenuCustomBL
    {
        ...
        public static List<clsMLOV> GetListLOVmMenuLOVPaging(string txtUserId, bool bitSuperUser, int intStart, int intEnd, string txtSearch, ref decimal decTotalRow)
        {
            KampusMerdekaEntities dObjContext = null;
            DbContextTransaction dObjTran = null;
            try
            {
                dObjContext = new KampusMerdekaEntities(EFClientUtility.GetConnectionString());
                return GetListLOVmMenuLOVPaging(txtUserId, bitSuperUser, intStart, intEnd, txtSearch, ref decTotalRow, dObjContext, dObjTran);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static List<clsMLOV> GetListLOVmMenuLOVPaging(string txtUserId, bool bitSuperUser, int intStart, int intEnd, string txtSearch, ref decimal decTotalRow, KampusMerdekaEntities dObjContext, DbContextTransaction dObjTran)
        {
            var retList = (from Item in dObjContext.mMenus
                           where 1 == 1
                                      && (txtSearch == ""
                                      || Item.intMenuID.ToString().Contains(txtSearch)
                                      || Item.txtMenuName.ToString().Contains(txtSearch)
                                      || Item.txtDescription.ToString().Contains(txtSearch)
                                      || Item.txtLink.ToString().Contains(txtSearch)
                                      )
                           select new clsMLOV
                           {
                               txtColumn1 = Item.intMenuID.ToString(),
                               txtColumn2 = Item.txtMenuName.ToString(),
                               txtColumn3 = Item.txtDescription.ToString(),
                               txtColumn4 = Item.txtLink.ToString(),
                               txtColumnName1 = "Menu ID",
                               txtColumnName2 = "Menu Name",
