...
namespace KN_KAMPUS_MERDEKA.BUSSLOGIC.CustomBL.Systems
{
    public static class mUserCustomBL
    {
        ...
        public static bool Login(string txtUserName, string txtPassword, string txtLangId, KampusMerdekaEntities dObjContext, DbContextTransaction dObjTran)
        {
            ...
            if (userDat != null)
            {
                ...
                if (userDat.bitUseActiveDirectory == true)
                {
                    ...
                    string txtDomain = mSystemConfigurationCustomBL.GetmSystemConfigurationValue(clsMMainConstant.MODULE_NAME, clsMMainConstant.CONFIGURATION.Key.DOMAIN, txtLangId, dObjContext, dObjTran);
        ...
        public static bool ResetPassword(mUser dat, string txtUserID, string txtLangId, string txtGUID, KampusMerdekaEntities dObjContext, DbContextTransaction dObjTran)
        {
            ...
            // Reset password.
            dat.txtPassword = CryptoEngine.MD5Encrypt(mSystemConfigurationCustomBL.GetmSystemConfigurationValue(clsMMainConstant.MODULE_NAME, clsMMainConstant.CONFIGURATION.Key.DefaultPassword, txtLangId, dObjContext, dObjTran));
        ...
        {
            mUser dat = new mUser();
            dat.bitActive = clsGlobal.ParseToBoolean(jsonDat["bitActive"].ToString());
            dat.bitUseActiveDirectory = clsGlobal.ParseToBoolean(jsonDat["bitUseActiveDirectory"].ToString());
            ...
            dat.dtmLastLogin = clsGlobal.ParseToDateTime(jsonDat["dtmLastLogin"].ToString());
            dat.intUserID = clsGlobal.ParseToInteger(jsonDat["intUserID"].ToString());
            dat.txtEmail = clsGlobal.ParseToString(jsonDat["txtEmail"].ToString());
            dat.txtEmpID = clsGlobal.ParseToString(jsonDat["txtEmpID"].ToString());
            dat.txtFullName = clsGlobal.ParseToString(jsonDat["txtFullName"].ToString());
            dat.txtNick = clsGlobal.ParseToString(jsonDat["txtNick"].ToString());
            dat.txtPassword = clsGlobal.ParseToString(jsonDat["txtPassword"].ToString());
            dat.txtUserName = clsGlobal.ParseToString(jsonDat["txtUserName"].ToString());
