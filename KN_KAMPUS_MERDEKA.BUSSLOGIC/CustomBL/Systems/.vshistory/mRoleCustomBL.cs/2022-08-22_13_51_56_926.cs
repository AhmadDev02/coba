...
namespace KN_KAMPUS_MERDEKA.BUSSLOGIC.CustomBL.Systems
{
    public static class mRoleCustomBL
    {
        ...
        public static List<clsMLOV> GetListLOVmRoleLOVPaging(string txtUserId, bool bitSuperuser, int intStart, int intEnd, string txtSearch, ref decimal decTotalRow)
        {
            KampusMerdekaEntities dObjContext = null;
            DbContextTransaction dObjTran = null;
            try
            {
                dObjContext = new KampusMerdekaEntities(EFClientUtility.GetConnectionString());
                return GetListLOVmRoleLOVPaging(txtUserId, bitSuperuser, intStart, intEnd, txtSearch, ref decTotalRow, dObjContext, dObjTran);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static List<clsMLOV> GetListLOVmRoleLOVPaging(string txtUserId, bool bitSuperuser, int intStart, int intEnd, string txtSearch, ref decimal decTotalRow, KampusMerdekaEntities dObjContext, DbContextTransaction dObjTran)
        {
            var retList = (from Item in dObjContext.mRoles
                           where 1 == 1
                                      && (txtSearch == ""
                                      || Item.intRoleID.ToString().Contains(txtSearch)
                                      || Item.txtRoleName.ToString().Contains(txtSearch)
                                      || Item.bitSuperuser.ToString().Contains(txtSearch)
                                      )
                           select new clsMLOV
                           {
                               txtColumn1 = Item.intRoleID.ToString(),
                               txtColumn2 = Item.txtRoleName.ToString(),
                               txtColumn3 = Item.bitSuperuser.ToString(),
                               txtColumnName1 = "Role ID",
                               txtColumnName2 = "Role Name",
                               txtColumnName3 = "Superuser"
                           }
                  ).AsNoTracking().ToList();
        ...
        public static List<mRole> GetAllmRolerByRoleID(int intRoleID, KampusMerdekaEntities dObjContext, DbContextTransaction dObjTran)
        {
            ...
            if (clsGlobal.ParseToInteger(intRoleID) != 0)
            {
        ...
        public static mRole CreateBlankmRole()
        {
            ...
            dat.dtmInsertedDate = clsGlobal.DATE_MINVALUE;
            dat.dtmUpdatedDate = clsGlobal.DATE_MINVALUE;
            dat.intRoleID = 0;
        ...
        public static mRole parseFromJSON(JObject jsonDat)
        {
            mRole dat = new mRole();
            dat.bitSuperuser = clsGlobal.ParseToBoolean(jsonDat["bitSuperuser"].ToString());
            dat.dtmInsertedDate = clsGlobal.ParseToDateTime(jsonDat["dtmInsertedDate"].ToString());
            dat.dtmUpdatedDate = clsGlobal.ParseToDateTime(jsonDat["dtmUpdatedDate"].ToString());
            dat.intRoleID = clsGlobal.ParseToInteger(jsonDat["intRoleID"].ToString());
            dat.txtInsertedBy = clsGlobal.ParseToString(jsonDat["txtInsertedBy"].ToString());
            dat.txtRoleName = clsGlobal.ParseToString(jsonDat["txtRoleName"].ToString());
            dat.txtUpdatedBy = clsGlobal.ParseToString(jsonDat["txtUpdatedBy"].ToString());
            dat.txtGUID = clsGlobal.ParseToString(jsonDat["txtGUID"].ToString());
        ...
        {
            SFERPSEntities dObjContext = null;
            DbContextTransaction dObjTran = null;
            try
            {
                dObjContext = new SFERPSEntities(EFClientUtility.GetConnectionString());
        ...
        public static bool IsExistBytxtGUID(string txtRoleName, string txtGUID, SFERPSEntities dObjContext, DbContextTransaction dObjTran)
        {
            mRole dat = new mRole();
        ...
        public static mRole GetBytxtGUID(string txtRoleName, string txtGUID, SFERPSEntities dObjContext, DbContextTransaction dObjTran)
        {
            dObjContext.Configuration.ProxyCreationEnabled = false;
            var retDat = (from item in dObjContext.mRoles
                          where 1 == 1
