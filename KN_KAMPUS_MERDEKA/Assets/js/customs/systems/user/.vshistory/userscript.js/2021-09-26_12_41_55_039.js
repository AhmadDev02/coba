...
function fillTableData() {
    ...
    if (pageUser.hasContent) {
        for (let i = 0; i < pageUser.content.length; i++) {
            ...
            let $btnUserRole = `<button class='btnUserRole  btn btn-outline-success float-right m-1' data-user='` + JSON.stringify(r) + `'> <i class='fa fa-keys'></i> Role</button>`;
        ...
        $(".btnUserRole").on('click', function () {
            ...
            //ROLEACCESS SCRIPT
            selectedUser = r;
            $userRoleFormTitle.text(roleParent.txtRoleName);
...
$btnCloseUserRole.bind('click', () => {
    ...
    $colRole.toggleClass('offset-md-3 col-sm-10 offset-sm-1', 200)
})
function searchUser() {
    pageUser.page = 1;
    getAllUser(pageUser.page, cari);
