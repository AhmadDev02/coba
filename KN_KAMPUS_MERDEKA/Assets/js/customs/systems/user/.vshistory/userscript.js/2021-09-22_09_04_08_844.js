...
function fillTableData() {
    ...
    if (pageUser.hasContent) {
        for (let i = 0; i < pageUser.content.length; i++) {
            ...
            let $btnEdit = `<button class='btnEdit  btn btn-outline-danger float-right m-1' data-user='` + JSON.stringify(u) + `'> <i class='fa fa-chevron-down'></i> Cancel</button>`;
...
function setupDataUserEditForm() {
    user.txtUserName = $txtUsername.val(user.txtUserName).toString();
    user.txtFullName = $txtFullname.val(user.txtFullName).toString();
    user.txtNick = $txtNick.val(user.txtNick).toString();
    user.txtEmpID = $txtEmployeeID.val(user.txtEmpID).toString();
    user.txtEmail = $txtEmail.val(user.txtEmail).toString();
    user.txtPassword = $txtPassword.val(user.txtPassword).toString();
    $bitActive.prop('checked', user.bitActive);
    $bitUseActiveDirectory.prop('checked', user.bitUseActiveDirectory);
}
