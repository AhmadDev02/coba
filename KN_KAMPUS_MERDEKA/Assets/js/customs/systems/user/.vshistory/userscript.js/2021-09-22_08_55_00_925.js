    ...
    if (pageUser.hasContent) {
        for (let i = 0; i < pageUser.content.length; i++) {
            ...
            let row = "<tr id='idUser" + u.intUserID + "'>" +
                ...
                "<button class='btnEdit  btn btn-outline-danger float-right m-1' data-u='" + JSON.stringify(udata) +"'> <i class='fa fa-chevron-down'></i> Cancel</button>"+
            "</td>" +
                "</tr>";
            $("#tbody").append(row);
        }
        $(".btnEdit").on('click', () => {
            console.log(JSON.stringify($(this).$element.data('u')))
        })
