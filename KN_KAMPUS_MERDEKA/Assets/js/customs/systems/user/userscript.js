...
function fillTableData() {
    ...
    if (pageUser.hasContent) {
        for (let i = 0; i < pageUser.content.length; i++) {
            ...
            let $btnUserRole = `<button class='btnUserRole  btn btn-outline-success float-right m-1' data-user='` + JSON.stringify(u) + `'> <i class='fa fa-key'></i> Role</button>`;
        ...
        $(".btnUserRole").on('click', function () {
            ...
            if (!$colUserRole.is(':visible')) {
                $colUser.toggleClass('col-md-12', 200).promise().done(function () {
                    //$colRoleAccess.slideToggle()
                    $colUserRole.toggle('slide', { direction: 'right', duration: 200 })
                });
...
$btnCloseUserRole.bind('click', () => {
    ...
    $colUser.toggleClass('col-md-12', 200)
})
function searchUser() {
    pageUser.page = 1;
    getAllUser(pageUser.page, cari);
