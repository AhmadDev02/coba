...
function fillTableDataRoleAccess() {
    ...
    if (pageRoleAccess.hasContent) {
        for (let i = 0; i < pageRoleAccess.content.length; i++) {
            ...
            let $btnDeleteRoleAccess = `<button class='btnRoleAccess  btn btn-outline-success float-right m-1' data-role='` + JSON.stringify(r) + `'> <i class='fa fa-keys'></i> Access</button>`;
