...
function fillTableData() {
    ...
    if (pageModule.hasContent) {
        for (let i = 0; i < pageModule.content.length; i++) {
            ...
            let $btnSelect= `<button class=' btn btn-outline-warning btnEdit ' data-module='` + JSON.stringify(m) + `'> <i class='fa fa-edit'></i> Edit</button>`;
...
function selectData(data) {
    parent.getSelectedAlkes(JSON.parse(data)); // pass it to parent page
