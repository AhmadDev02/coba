...
let api = "/Transaction/Rps/Simulasi/SimulasiWeekSelect";
let alergen = {};
...
function getAllSimulasiWeekSelect(x, y) {
    $.postApi(api + "/getAllSimulasiWeekSelect",
        {
            page: x,
            cari: y,
            size: 10
...
function fillTableData() {
    ...
    if (pageSimulasiWeekSelect.hasContent) {
        for (let i = 0; i < pageSimulasiWeekSelect.content.length; i++) {
            ...
            let row = "<tr id='intSimulasiWeekSelectID" + m.intSimulasiWeekSelectID + "'>" +
                "<td class='text-center'>" +
                $btnSelect +
                "</td>" +
                "<td class='text-center'>" + m.intSimulasiWeekSelectID + "</td>" +
                "<td class='text-center'>" + m.txtSimulasiWeekSelectCode + "</td>" +
                "<td class='text-center'>" + m.txtSimulasiWeekSelectDescription + "</td>" +
                "</tr>";
            $("#tbodyLOV").append(row);
        }
