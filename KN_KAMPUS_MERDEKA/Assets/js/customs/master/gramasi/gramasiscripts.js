...
function getAllGramasi(x, y) {
    const augerVariant = parseInt($("#intProductVariantAugerID").val().toString());
    $.postApi(api + "/getAllGramasi",
        {
            page: x,
            cari: y,
            size: 20,
            augerVariant: augerVariant
        },
        function (response) {
...
function fillTableData() {
    ...
    if (pageGramasi.hasContent) {
        for (let i = 0; i < pageGramasi.content.length; i++) {
            ...
            let row = "<tr id='intGramasiID" + m.intProductVariantAugerGramasiID + "'>" +
                "<td class='text-center'>" + m.intProductVariantAugerGramasiID + "</td>" +
                "<td class='text-center'>" + m.txtProductVariantDescription + "<span class='text-danger'> " + m.intAugerSize + "</span><span class='text-success text-bold'>" + " " + m.decGramasi + " G" + "</span></td>" +
...
function setupDataGramasi() {
    gramasi.decGramasi = parseFloat($decGramasi.val().toString());
    gramasi.intProductVariantAugerID = parseInt($("#intProductVariantAugerID").val().toString());
}
//=======================
// ASSIGN EDIT PRODUCT
//=======================
...
function setupDataGramasiEditForm() {
    $decGramasi.val(gramasi.decGramasi.toString());
}
