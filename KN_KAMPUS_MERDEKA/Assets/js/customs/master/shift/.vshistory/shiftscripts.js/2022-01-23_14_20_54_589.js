    ...
    if (pageShift.hasContent) {
        for (let i = 0; i < pageShift.content.length; i++) {
            ...
            let row = "<tr id='intShiftID" + m.intShiftID + "'>" +
                ...
                `<td class='text-center bg-warning'> <span class="text-success"> ${m.intKapasitas} Menit</span></td>`+
                `<td class='text-center bg-warning'> <span class='text-muted'>(${(m.intKapasitas / 60).toFixed(2)}) Jam</span></td>` +
