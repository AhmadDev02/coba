...
function getSelectedProductVariantAugerGramasi(x) {
    selectedGramasi = x;
    $txtSelectedGramasi.val(`${selectedGramasi.txtProductVariantDescription} ${selectedGramasi.decGramasi} GR`)
}
...
function fillTableData() {
    ...
    if (pageItem.hasContent) {
        for (let i = 0; i < pageItem.content.length; i++) {
            ...
            //let $btnSetup = !m.bitSetup ? `<button class=' btn btn-outline-warning btn-setup' data-item='` + JSON.stringify(m) + `'> <i class='fa fa-edit'></i> Setup</button>` : `<button class=' btn btn-info btn-edit' data-item='` + JSON.stringify(m) + `'> <i class='fa fa-edit'></i> Edit</button>`;
            let $btnSetup = !m.bitSetup ? `<button class=' btn btn-outline-warning btn-setup' data-item='` + JSON.stringify(m) + `'> <i class='fa fa-edit'></i> Setup</button>` : `No Action`;
            let row = "<tr id='intItemID" + m.intItemID + "'>" +
                ...
                "<td class='text-center'>" + m.txtAlergen + "</td>" +
        ...
        $(".btn-edit").on('click', () => {
            let x = $(this).data('item');
            selectedSetupItem = x;
            EditItemForm();
            $.switchElement(elementToSwitch, 0);
            $setupTitle.html(`Edit Item <span class='text-green text-bold blinking'>${x.txtItemDescription}</span>`);
        })
    ...
    if (selectedAlergen == null) {
        ...
        if (selectedGramasi == null) {
            ...
            if (selectedSetupItem == null) {
                ...
                $.confirmMessageHtml(`${selectedSetupItem.txtItemDescription}`,
                    `
                    <div class='table-responsive'>
                    <table class='table table-striped table-hover table-bordered table-warning'>
                        <thead  class="bg-info">
                            <tr>
