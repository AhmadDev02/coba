        ...
        for (let i = 0; i < pageBo.content.length; i++) {
            ...
            let $switcHold = fullacces ?
                `<div class="form-group">
                                <label class="switch">
                                    <input type="checkbox" class="bitHold" id="bithold-`+ idIntBo + `" data-bo='` + JSON.stringify(m) + `' conclick="(function(e){ e.preventDefault(); setHold(e); })(event)">
            ...
            let $switcPrioritas = fullacces ?
                `<div class="form-group">
                                <label class="switch">
                                    <input type="checkbox" class="bitPrioritas" id="bitprioritas-`+ idIntBo + `" data-bo='` + JSON.stringify(m) + `' onchange="(function(e){ e.preventDefault(); setHold(e); })(event)">
                                    <span class="slider"></span>
                                </label>
...
function setHold(e) {
    ...
    let $switchHold = e.target;
    console.log($switchHold.prop("isChecked"));
    try {
        if (boTemp.intBoID) {
            ...
            $.confirmMessage(textConfirm, "Apakah Yakin?", "Saya Yakin", function () {
                ...
                $("#bithold-" + boTemp.idIntBo).prop('checked', boTemp.bitHold);
            })
        }
