...
let $intKapasitasJam = $("#intKapasitasJam");
    ...
    if (pageShift.hasContent) {
        for (let i = 0; i < pageShift.content.length; i++) {
            ...
            let row = "<tr id='intShiftID" + m.intShiftID + "'>" +
                ...
                "<td class='text-center'>" + m.dtmStartTime.substring(0, 5) + "</td>" +
                "<td class='text-center'>" + m.dtmEndTime.substring(0, 5) + "</td>" +
                `<td class='text-center bg-white'> <span class="text-success"> ${m.intKapasitas} Menit</span></td>` +
                `<td class='text-center bg-secondary'> <span class='text-white'>(${(m.intKapasitas / 60).toFixed(2)} Jam)</span></td>` +
