...
function fillTableData() {
    ...
    if (pageBo.hasContent) {
        for (let i = 0; i < pageBo.content.length; i++) {
            ...
            let currentBitHold = response.objData.content[i].bitHold;
            let idIntBo response.objData.content[i].intBoUrtSpec;
            let $badge = currentBitTrial ? "bg-red" : "bg-green";
            let $badgeText = currentBitTrial ? "Trial" : "Sales";
            let $switcTrial = fullacces ?
