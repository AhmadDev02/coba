...
function fillTableData() {
    ...
    if (pageProductVariantAuger.hasContent) {
        for (let i = 0; i < pageProductVariantAuger.content.length; i++) {
            ...
            let $btnGramasi = `<a href='/master/gramasi/` + m.intProductVariantAugerID + `' class=' btn btn-outline-success btnGramasi m-1' data-productvariantauger='` + JSON.stringify(m) + `'> <i class='fa fa-balance-scale'></i> Gramasi</button>`;
