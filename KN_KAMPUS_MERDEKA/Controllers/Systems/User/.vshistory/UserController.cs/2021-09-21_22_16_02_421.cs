        ...
        public ActionResult Save(UserRegister userRegister)
        {
            try
            {
                ...
                mUser user = new mUser(userRegister);
                    if (mUserCustomBL.IsExistMUser(user.intUserID))
                    {
                        //Update 
                        bitSuccess = mUserCustomBL.UpdateMUser(user, GlobalClass.dLogin.userDat.intUserID.ToString(), GlobalClass.dLogin.txtLangID, txtGUID);
                    ...
                    {
                        //Create 
                        user.intUserID = mUserCustomBL.SavemUser(objDat, GlobalClass.dLogin.userDat.intUserID.ToString(), GlobalClass.dLogin.txtLangID, txtGUID);
                        txtStatus = mSystemLanguageCustomBL.GetmSystemLanguageValue(clsMMainConstant.MODULE_NAME, clsMMainConstant.LANGUAGE.MSG_INSERT_DATA, GlobalClass.dLogin.txtLangID);
                        bitSuccess = true;
                    }
                return Json(clsAPI.CreateResult(bitSuccess, objDat, txtStatus, string.Empty));
