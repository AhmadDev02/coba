...
namespace KN_KAMPUS_MERDEKA.MVC.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult InitiateData()
        {
            try
            {
                throw new Exception("eew")
                var json = Json(new SuccessResponse<UserRegister>());
                return json;
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Save(UserRegister userRegister)
        {
            try
            {
                ...
                else
                {
                    ...
                    user.intUserID = mUserCustomBL.SavemUser(user, userLogin, CurrentSession.getPrincipal.txtLangID, userRegister.txtGUID);
                    txtStatus = mSystemLanguageCustomBL.GetmSystemLanguageValue(Configuration.MODULE_NAME, Configuration.LANGUAGE.MSG_INSERT_DATA, CurrentSession.getPrincipal.txtLangID);
                    bitSuccess = true;
                }
                return Json(new SuccessResponse<UserRegister>(userRegister));
            }
            catch (Exception ex)
            {
                Response.StatusCode = (int)HttpStatusCode.BadRequest;
