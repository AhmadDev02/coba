        ...
        public ActionResult Save(ModuleRequest moduleRequest)
        {
            try
            {
                ...
                if (mModuleCustomBL.IsExistMModule(moduleRequest.intModuleID) && moduleRequest.intModuleID != 0)
                {
                    ...
                    //Update 
                    bitSuccess = mModuleCustomBL.UpdateMModule(savedModule, userLogin, CurrentSession.getPrincipal.txtLangID, savedModule.txtGUID);
                ...
                else
                {
                    ...
                    module.intModuleID = mModuleCustomBL.SaveMModule(module, userLogin, CurrentSession.getPrincipal.txtLangID, moduleRequest.txtGUID);
                    txtStatus = mSystemLanguageCustomBL.GetmSystemLanguageValue(Configuration.MODULE_NAME, Configuration.LANGUAGE.MSG_INSERT_DATA, CurrentSession.getPrincipal.txtLangID);
                    bitSuccess = true;
                }
                return Json(new SuccessResponse<ModuleRequest>(moduleRequest));
            }
            catch (Exception ex)
            {
                Response.StatusCode = (int)HttpStatusCode.BadRequest;
