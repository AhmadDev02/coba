        ...
        public ActionResult Save(MenuRequest menuRequest)
        {
            try
            {
                ...
                else
                {
                    ...
                    menu.intMenuID = mMenuCustomBL.SaveMMenu(menu, userLogin, CurrentSession.getPrincipal.txtLangID, menuRequest.txtGUID);
                    txtStatus = mSystemLanguageCustomBL.GetmSystemLanguageValue(Configuration.MODULE_NAME, Configuration.LANGUAGE.MSG_INSERT_DATA, userLogin);
                    bitSuccess = true;
                }
                return Json(new SuccessResponse<MenuRequest>(menuRequest));
            }
            catch (Exception ex)
            {
                Response.StatusCode = (int)HttpStatusCode.BadRequest;
