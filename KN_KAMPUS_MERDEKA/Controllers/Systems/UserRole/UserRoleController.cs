        ...
        public ActionResult Save(UserRoleRequest userRoleRequest)
        {
            try
            {
                ...
                else
                {
                    ...
                    userRole.intUserRoleID = mUserRoleCustomBL.SaveMUserRole(userRole, userLogin, CurrentSession.getPrincipal.txtLangID, userRole.txtGUID);
                    txtStatus = mSystemLanguageCustomBL.GetmSystemLanguageValue(Configuration.MODULE_NAME, Configuration.LANGUAGE.MSG_INSERT_DATA, CurrentSession.getPrincipal.txtLangID);
                    bitSuccess = true;
                }
                return Json(new SuccessResponse<UserRoleRequest>(userRoleRequest));
            }
            catch (Exception ex)
            {
                Response.StatusCode = (int)HttpStatusCode.BadRequest;
